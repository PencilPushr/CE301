...
public:
	//TODO: change it so that we also have constexprs for PE file based on section headers.
	// currently a macro for testing, because we know the file name at compile time and therefore set the macro
#ifdef PE_FILE
	//define PE sections (e.g. .text = 0x0000 -> 0x2556, .rodata 0x0x2556 -> 0x2316 -> ...etc.
	static constexpr int startAddress 0x0;
	static constexpr int endAddress startAddress + PE_FILE_SIZE;
#else //use the i8080 memory division
	static constexpr int TotalAdrSpaceLwrBnd = 0x0000;
	static constexpr int TotalAdrSpaceUprBnd = MEMORYMAXSIZE;
...
public:
	...
	template<uint8_t Bytes = DefaultBytes, Type DataLoc = DefaultType>
	...
	template<uint8_t PtrBytes = DefaultBytes, uint8_t DataBytes = DefaultBytes, Type PtrLoc = DefaultType, Type DataLoc = DefaultType>
	...
	/*
		ByteSize must be prefaced with typename, this because ByteSize<>::Type is a dependant name -> it is dependant on ByteSize.
		What this means, I have no idea. dont ask me.
		Something something, bytesize is a template argument so we have to specify that before or the compiler has a mental breakdown.
	*/
	template<uint8_t Bytes = DefaultBytes, Type DataLoc = DefaultType>
	void DirectWriteBytes(Ptr offsetWritingTo, typename ByteSize<Bytes>::Type dataToWrite, ErrorCode& e)
	{
...
public:
	//uint8_t* m_Memory;
	//TODO: do not allocate 4 billion bytes on stack. (it is only a SMALL allocation of memory)
	std::array<uint8_t, MEMORYMAXSIZE> m_Memory = { 0 };
	std::unordered_map<std::string, Section<Ptr>> m_Sections;
};
